...
async def test_notification_forwarding():
    ...
    async def test_callback(session: ClientSession):
        ...
        result = await session.call_tool(
            ...
            arguments={"include_invalid": True}
        )
        ...
        for expected_method in expected_valid_notifications:
            assert expected_method in received_methods, f"Missing expected notification: {expected_method}"
        
        # Check that invalid notification was NOT received
        assert invalid_notification not in received_methods, f"Invalid notification was forwarded: {invalid_notification}"
        ...
        assert not unexpected_notifications, f"Received unexpected notifications: {unexpected_notifications}"
...
async def test_notification_forwarding_without_invalid():
    ...
    async def test_callback(session: ClientSession):
        ...
        result = await session.call_tool(
            name="send_notifications",
            arguments={"include_invalid": False}
        )
        ...
        assert received_methods == expected_valid_notifications, f"Expected {expected_valid_notifications}, got {received_methods}"
    ...
    async def test_callback(session: ClientSession):
        ...
        message_notification = types.LoggingMessageNotification(
            ...
            params=types.LoggingMessageNotificationParams(
                level="info",
                data="Test log message from client",
                logger="test-client"
            )
        )
        ...
        assert "notifications/initialized" in received_methods, "Should receive initialized notification"
        ...
        logger.info(f"✅ Verified client→server notification forwarding works (received {received_count} notifications)")
