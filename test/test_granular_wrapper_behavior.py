...
async def test_granular_tool_filtering_in_list_tools():
    ...
    await run_with_wrapper_session(
        ...
        "python contextprotector/test/multi_tool_downstream_server.py", 
        temp_file.name
    )
    
    # Approve the server
    await approve_server_config_using_review(
        "stdio", "python contextprotector/test/multi_tool_downstream_server.py", temp_file.name
    )
    ...
    config = db.get_server_config("stdio", "python contextprotector/test/multi_tool_downstream_server.py")
    
    # Reset approval - remove the server entry completely and recreate it
    from contextprotector.mcp_config import MCPServerEntry
    key = MCPServerEntry.create_key("stdio", "python contextprotector/test/multi_tool_downstream_server.py")
    if key in db.servers:
        del db.servers[key]
        db._save()
    
    # Save as unapproved config (this will create a fresh entry)
    db.save_unapproved_config("stdio", "python contextprotector/test/multi_tool_downstream_server.py", config)
    ...
    db.approve_instructions("stdio", "python contextprotector/test/multi_tool_downstream_server.py", config.instructions)
    if echo_tool:
        db.approve_tool("stdio", "python contextprotector/test/multi_tool_downstream_server.py", "echo", echo_tool)
    ...
    async def test_granular_filtering(session):
        ...
        assert "greet" not in tool_names, f"Unapproved greet tool should not be visible, got: {tool_names}"
        ...
        try:
            ...
            assert "not approved" in response_json["reason"].lower(), "Should indicate tool not approved"
...
async def test_tool_modification_blocks_only_modified_tool():
    ...
    tool1 = MCPToolDefinition(
        ...
        parameters=[
            MCPParameterDefinition(name="input1", description="Input 1", type=ParameterType.STRING, required=True)
        ]
    )
    ...
    tool2 = MCPToolDefinition(
        ...
        parameters=[
            MCPParameterDefinition(name="input2", description="Input 2", type=ParameterType.STRING, required=True)
        ]
    )
    ...
    print("✅ Tool modification correctly detected - stable tool remains approved, modified tool needs re-approval")
...
async def test_instruction_change_blocks_everything():
    ...
    print("✅ Instruction change correctly detected - wrapper will block all tools despite individual tool approval")
...
async def test_tool_removal_workflow():
    ...
    keep_tool = MCPToolDefinition(
        ...
        parameters=[MCPParameterDefinition(name="input", description="Input", type=ParameterType.STRING, required=True)]
    )
    ...
    print("✅ Tool removal works correctly - remaining tools stay approved, removed tools disappear")
